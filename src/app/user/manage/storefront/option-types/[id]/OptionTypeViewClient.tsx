...
export default function OptionTypeViewClient({ object, items }: { object: HydratedOptionType, items: ItemType[] }) {
    ...
    return <>
        <Modal show={deleteModal} onClose={() => setDeleteModal(false)}>
            ...
            <ModalBody>
                <p>{t('manage.storefront.optionTypeD.deleteMessage')}</p>
            </ModalBody>
            <ModalFooter>
                <Button pill color="failure"
                        ...
                        onClick={async () => {
                            setLoading(true)
                            await deleteOptionType(object.id)
                            setLoading(false)
        ...
        <div className="container">
            ...
            <div className="2xl:w-1/2 mb-5">
                <div className="bg-amber-50 dark:bg-amber-900 rounded-3xl p-5 mb-3"
                    ...
                    <p className="secondary text-sm font-display">{t('manage.storefront.optionTypeD.id')}</p>
                    <p className="text-xl mb-3">{object.id}</p>

                    <p className="secondary text-sm font-display">{t('manage.storefront.optionTypeD.name')}</p>
